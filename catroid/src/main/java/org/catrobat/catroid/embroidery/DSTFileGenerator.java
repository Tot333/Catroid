...
public class DSTFileGenerator {
	...
	*/
	...
	private String createHeaderString() {
		...
		String label = String.format("LA:%-15s\n" + substituteChar, ProjectManager.getInstance().getCurrentProject().getName());
		String header = "ST:%-6d\n" + substituteChar + "CO:%-2d\n" + substituteChar
				+ "+X:%-4d\n" + substituteChar + "-X:%-4d\n" + substituteChar
				+ "+Y:%-4d\n" + substituteChar + "-Y:%-4d\n" + substituteChar
				+ "AX:%-5d\n" + substituteChar + "AY:%-5d\n" + substituteChar
				+ "MX:%-5d\n" + substituteChar + "MY:%-5d\n" + substituteChar
				+ "PD:%-5s\n" + substituteChar;

		return label + String.format(Locale.getDefault(), header, stitchPoints.size(), colorChanges,
