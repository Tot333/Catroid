...
public class EmbroideryActor extends Actor {
	...
	public EmbroideryActor() {
		stitchSize = getPenSizeForScreen();
	...
	private float getPenSizeForScreen() {
		DisplayMetrics metrics = Resources.getSystem().getDisplayMetrics();
		float one_inch = 25.4f;
		float base = 0.5f / one_inch;
		float dpi = metrics.densityDpi;

		float heightxheightmm = (metrics.heightPixels / dpi) * (metrics.heightPixels / dpi);
		float widthxwitdhmm = (metrics.widthPixels / dpi) * (metrics.widthPixels / dpi);
		float display_diagonal = (float) Math.sqrt(heightxheightmm * widthxwitdhmm);

		// resolution of the creator device
		Project project = ProjectManager.getInstance().getCurrentProject();
		int pw = project.getXmlHeader().getVirtualScreenWidth();
		int ph = project.getXmlHeader().getVirtualScreenHeight();

		float thickness = (float) Math.sqrt(pw * pw + ph * ph) / display_diagonal;
	...
	private void drawStitchPoints(Sprite sprite) {
		...
		if (embroidery.stitch) {
			...
			if (embroidery.stitch && (embroidery.previousPoint.x != sprite.look.getX() || embroidery.previousPoint.y != sprite.look.getY())) {
				renderer.circle(embroidery.previousPoint.x, embroidery.previousPoint.y, stitchSize / 2);
				renderer.rectLine(embroidery.previousPoint.x, embroidery.previousPoint.y, x, y, stitchSize);
				renderer.circle(x, y, stitchSize);
			}
